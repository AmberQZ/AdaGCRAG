...
class adarag:
    working_dir: str = field(
        default_factory=lambda: f"./adarag_cache_{datetime.now().strftime('%Y-%m-%d-%H:%M:%S')}"
    )
    ...
    def __post_init__(self):
        log_file = os.path.join(self.working_dir, "adarag.log")
        set_logger(log_file)
        ...
        logger.debug(f"adarag init with param:\n  {_print_config}\n")
    ...
    async def aquery(self, query: str, param: QueryParam = QueryParam()):
        if param.mode == "light":
            print("light mode is not supported yet")
